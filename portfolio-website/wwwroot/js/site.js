...
// === Download Resume (Frontend Simulation) ===

function initiateDownload() {
    // State 1: Show downloading state
    showModal(
        'Downloading Resume...', 
        '<p class="text-center text-slate-700 dark:text-slate-300">Preparing your resume PDF. Please wait...</p>' +
        '<div class="w-full bg-slate-200 dark:bg-slate-700 h-2 mt-4 rounded-full overflow-hidden">' +
        '<div class="bg-purple-vibrant h-full w-1/2 animate-pulse"></div></div>'
    );
    ...
    setTimeout(() => {
        showModal(
            ...
            '<i data-lucide="check-circle" class="w-12 h-12 mx-auto"></i></div>' +
            '<p class="text-center font-bold text-slate-900 dark:text-white">' +
            'Your resume has been downloaded successfully.</p>'
        );
        lucide.createIcons();
...
function showProject(key) {
    fetch(`/Home/GetProjectDetails?id=${key}`)
        ...
        .then(data => {
            if (data) {
                // Format technology tags
                const tags = data.technologies
                    .map(t => `<span class="px-2 py-1 bg-purple-50 dark:bg-slate-800 text-purple-deep dark:text-purple-200 text-xs font-bold rounded-md border border-purple-100 dark:border-slate-700">${t}</span>`)
                    .join('');
